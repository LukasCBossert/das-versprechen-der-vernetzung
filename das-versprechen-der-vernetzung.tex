...
Als Datengrundlage nehmen wir die \emph{Letter of Intent} der jeweiligen
Konsortien, in denen Kooperationspartner genannt werden. Diese Nennungen
sind Ausgangspunkt unseres Netzwerkes.\footnote{Siehe dazu auch das
  Repositorium von Dorothea Strecker
...
Sie zeigen eine Verbindung zwischen zwei Knoten an. Kreuzungen
(\emph{crossings}) sind Schnittpunkte zweier oder mehrerer Kanten.
...
Funktionen geladen werden müssen. Für die Netzwerkanalyse werden wir auf
das Paket \texttt{igraph}\footnote{https://igraph.org/r/} zurückgreifen.
Mit \texttt{library(\textquotesingle{}igraph\textquotesingle{})} können
wir das Paket laden.
...
Fangen wir bei der Funktion \texttt{read.table} an. Wir geben an, dass
es sich um einen Datensatz handelt, bei dem es

\begin{itemize}
\tightlist
\item
  eine Kopfzeile gibt, daher \texttt{header=TRUE}
\item
  die Trennung der Werte durch ein Komma erfolgt, \texttt{sep=","}.
\item
...
es aufbereiten und ein \texttt{igraph\ graph} erstellen.\footnote{https://igraph.org/r/doc/graph\_from\_data\_frame.html}
...
\texttt{seed}. Wir wählen eine beliebige Zahl, die nicht allzu klein
ist.
...
\PY{n+nf}{plot}\PY{p}{(}\PY{n}{NFDI\PYZus{}network}\PY{p}{,}                    \PY{c+c1}{\PYZsh{} loading data frame}
     ...
     \PY{n}{frame} \PY{o}{=} \PY{k+kc}{TRUE}                     \PY{c+c1}{\PYZsh{} FALSE \PYZhy{}\PYZgt{} making a frame }
     \PY{p}{)}
\end{Verbatim}
\end{tcolorbox}
...
Datensatz kann mal das eine, mal ein anderes besser geeignet sein.
Meiner Meinung nach erzielen wir ein gutes Ergebnis mit dem Layout
\texttt{graphopt}\footnote{https://igraph.org/r/doc/layout\_with\_graphopt.html}.

Dieses Layout übergeben wir dem Parameter \texttt{layout} mit dem Wert
\texttt{layout.graphopt}.
...
\PY{n+nf}{plot}\PY{p}{(}\PY{n}{NFDI\PYZus{}network}\PY{p}{,}                     \PY{c+c1}{\PYZsh{} loading data frame}
     \PY{n}{main}   \PY{o}{=} \PY{l+s}{\PYZdq{}}\PY{l+s}{NFDI\PYZhy{}Netzwerk\PYZdq{}}\PY{p}{,}         \PY{c+c1}{\PYZsh{} adding a title}
     \PY{n}{frame}  \PY{o}{=} \PY{k+kc}{TRUE}\PY{p}{,}                    \PY{c+c1}{\PYZsh{} FALSE \PYZhy{}\PYZgt{} making a frame}
...
\tightlist
\item
  \texttt{layout.auto}: Choose an appropriate graph layout algorithm
  automatically
\item
  \texttt{layout.davidson.harel}: The Davidson-Harel layout algorithm
\item
  \texttt{layout.drl}: The DrL graph layout generator
\item
  \texttt{layout.fruchterman.reingold}: Deprecated layout functions
\item
  \texttt{layout.gem}: The GEM layout algorithm
\item
  \texttt{layout.graphopt}: The graphopt layout algorithm
\item
  \texttt{layout.grid}: Simple grid layout
\item
  \texttt{layout.mds}: Graph layout by multidimensional scaling
\item
  \texttt{layout.merge}: Merging graph layouts
\item
  \texttt{layout.norm}: Normalize coordinates for plotting graphs
\item
  \texttt{layout.star}: Generate coordinates to place the vertices of a
  graph in a star-shape
\end{itemize}

    \hypertarget{farbe-gruxf6uxdfe-kurve-knoten-und-kanten}{%
\subsection{Farbe, Größe, Kurve (Knoten und
Kanten)}\label{farbe-gruxf6uxdfe-kurve-knoten-und-kanten}}

Nachdem wir die Anordnung der Knoten optimiert haben, wollen wir im
nächsten Schritt die Darstellung der Knoten und Kanten selbst angehen.
...
Ein Netzwerk besteht neben den Knoten auch aus Kanten, die
Verbindungslinien, die wir ebenfalls modifizieren können. Für die
Farbänderung brauchen wir den Parameter \texttt{edge.color}, an den wir
bspw. \texttt{"\#808080"} übergeben. Neben der Farbe können wir auch den
Grad der ``Kurvigkeit'' bestimmen, die mit \texttt{edge.curved} und dem
Wert \texttt{0.1} eingestellt wird. Wichtig ist auch hier wieder, dass
\emph{keine} Anführungszeichen gesetzt werden.
...
\end{tcolorbox}
    ...
    \hypertarget{knotengruxf6uxdfe-in-abhuxe4ngigkeit-der-kanten}{%
\subsection{Knotengröße in Abhängigkeit der
Kanten}\label{knotengruxf6uxdfe-in-abhuxe4ngigkeit-der-kanten}}
...
\end{description*}
    ...
    \hypertarget{knotengruxf6uxdfe-in-abhuxe4ngigkeit-der-ein--und-ausgehenden-kanten}{%
\subsection{Knotengröße in Abhängigkeit der ein- und ausgehenden
Kanten}\label{knotengruxf6uxdfe-in-abhuxe4ngigkeit-der-ein--und-ausgehenden-kanten}}
...
Bislang war es unerheblich ob ein Konsortium im Datensatz an erster oder
zweiter Stelle genannt wurde, das heißt, es war unerheblich ob der
...
Im gerichteten Netzwerk erschwert die Kurvigkeit der Kanten die
Lesbarkeit. Daher wäheln wir für \texttt{edge.curved} den Wert
\texttt{0}.
...
Förderrunde bereits bewilligt wurden und daher keinen neuen Letter of
Intent eingereicht haben. Unser Datensatz berücksichtigt ja nur die
Letter of Intent der zweiten Förderrunde. Die Konsortien der ersten
...
eine Berechnung statt: Es sollen alle Knoten/Konsortien gelöscht werden,
deren Anzahl an Kanten (\texttt{V} = Value) im Modus \texttt{out} gleich
\texttt{0} ist. Diese gelöschte Knoten übergeben wir der neuen Variable
...
Auffällig ist, dass die Naturwissenschaften auf die
Lebenswissenschaften, Ingenieurwissenschaften und Chemie/Physik
...
die Konferenzsystematik laden wir als fertigen Datensatz aus einem
GitHub-Gist, was uns manche Tipparbeit sparen wird.\footnote{https://gist.github.com/LukasCBossert/36d6034c8ebc2a4d61f011169371bc31}
...
dass wir nun differenzieren, was unser Kanten-Data-Frame ist und was die
...
hinter der Farbcodierung an sich steckt.
...
(\texttt{c(NA,"\#bf4040")}): Der erste Eintrag ist \texttt{NA}
(\texttt{Not\ Available}), womit wir die cis-disziplinäre Kanten
ansteuern; sie werden also nicht ausgegeben. Der zweite Einträg ist ein
HTML-Farbcode, den wir für die trans-disziplinäre Kanten verwenden. Die
Unterscheidung zwischen cis- und trans-disziplinärer Kante wird über die
Funktion \texttt{crossing} vorgenommen.
...
Beides könnt ihr selbst ausprobieren unter:
https://nfdi4ing.de/nfdi-network\\
Das Passwort lautet \texttt{fdmrwth}.
